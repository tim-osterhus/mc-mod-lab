...
This is a Windows Minecraft 1.21.1 Fabric Mojmap dev-instance alpha. The packaged click, world-path, `E` key, controlled time-command, and player-info paths were observed there, not in a published intermediary production JAR. Live doctor and a vanilla written-book `use_item` capture passed; its full-frame after-image was independently inspected. Mod parity is not claimed.
