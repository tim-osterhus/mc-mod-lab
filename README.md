...
The checked-in [auth patch](patches/minecraft-mod-mcp-v0.3.0-loopback-auth.patch), [framebuffer patch](patches/minecraft-mod-mcp-v0.3.0-framebuffer.patch), [dev-click patch](patches/minecraft-mod-mcp-v0.3.0-dev-click.patch), and [rebuild recipe](scripts/harden-bridge.ps1) cover the narrow local derivative. A Windows dev-instance QA run verified a full 1280x720 title screenshot with `lab2.jar` and title-to-disposable-world GUI navigation with `lab3.jar` (SHA-256 `0bda1e57a1d5c68711536f6a9c1b6462628d2a2bc097d609ddd480a1aa948ad0`). The lab3 world/player getters returned placeholder or incorrect fields, so this toolkit reports that client as unsupported. A cropped screenshot fails the declared minimum dimensions; no in-world toolkit or visual parity pass is claimed.

The alpha runner requires a bridge response containing the actual canonical world save path. The [dev-integration patch](patches/minecraft-mod-mcp-v0.3.0-dev-integration.patch) adds that getter and the verified dev key/command routes. `get_player_info` still reported a blank name and incorrect game mode in `lab5.jar`, so `doctor` must remain `unsupported` until the player getter is corrected and independently checked. This CLI does not expose command execution.

The v0.3.0 `/api/status` `version` field is sourced from `mcp.mod.version`, not a reliable Minecraft game-version field. The client PID, port, socket bind, world name, and canonical world save path are checked live. On Windows the CLI additionally checks the selected Java process command line for the declared `game_dir`, requires the marked fixture under that directory's `saves`, and requires a fresh launch log containing `Loading Minecraft 1.21.1 with Fabric Loader`. A verified local dev bridge revision exposed the actual integrated-server save path; the stock world getter does not. A successful capture is **not** visual or parity approval.
...
The [scenario](examples/scenario.json) permits one screen `click`, `click_button_index`, or one key from a small allowlist. A generic proposed in-world check is [open inventory](examples/inventory-scenario.json); change its world name to match the fixture. Its `press_key` path has not yet passed a live run. The runner enters MCP control mode for the action and attempts to restore manual control even on failure. It waits at most two seconds for an expected screen class before the after-capture. `click` acknowledgement is not proof that a widget activated; assert an after-screen class and inspect the PNG. The report records blank button labels and duplicate widget counts without raw labels. `tracked_pids` is the guarded Minecraft/build-tool group, capped at 3800 MiB for this checkpoint. Optional `informational_pids` can include Codex/ChatGPT app processes; their combined telemetry is reported but never blocks the run. These are point samples, not a cold-build peak guarantee, and this alpha does not promise low-RAM operation. Build, original client, and modern client must run sequentially. A missing, too-small, or inconsistent screenshot produces `fail`; an unverified identity or unsafe binding produces `unsupported`. `report.json` and `report.md` are written in either case when the output path is writable. PNGs require independent inspection.

The [parity record](examples/parity.json) keeps reference provenance, deterministic tests, client check, and visual check independent. Its example statuses are not verified claims. Document deliberate substitutions and exceptions; asset presence or a green unit-test suite does not imply gameplay parity.
...
3. Run `doctor`, then a harmless GUI scenario. Inspect both PNGs and state, test a known-failing case and its corrected counterpart, and retain the reports. No real-client result is claimed by this repository yet.
...
This is a Windows Minecraft 1.21.1 Fabric Mojmap dev-instance alpha. The packaged click, world-path, `E` key, and controlled time-command paths were observed there, not in a published intermediary production JAR. Player-info truth and a complete toolkit capture remain unresolved; no mod parity is claimed.
