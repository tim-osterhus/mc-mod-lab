...
The alpha runner requires a bridge response containing the actual canonical world save path. The [dev-integration patch](patches/minecraft-mod-mcp-v0.3.0-dev-integration.patch) adds that getter and the verified dev key/command routes. A separately tested local lab6 getter returned the actual player name and creative mode; that source change is not yet packaged here. This CLI does not expose command execution.
...
This is a Windows Minecraft 1.21.1 Fabric Mojmap dev-instance alpha. The packaged click, world-path, `E` key, and controlled time-command paths were observed there, not in a published intermediary production JAR. A local lab6 player-info fix and `use_item` control action were observed separately; the first complete toolkit capture remains pending. No mod parity is claimed.
