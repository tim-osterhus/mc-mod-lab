...
The alpha runner requires a bridge response containing the actual canonical world save path. The three patches packaged here do not yet add that getter; the separately tested local lab4 getter will be incorporated as a subsequent reviewed checkpoint. Until then, `doctor` must report the packaged derivative as unsupported for in-world runs.
...
This is a Windows Minecraft 1.21.1 Fabric dev-instance alpha. The packaged click fallback was observed in that Mojmap dev runtime, not in a published intermediary production JAR. World identity, key input, and command execution remain unverified or failing bridge capabilities; the toolkit does not claim to repair them.
